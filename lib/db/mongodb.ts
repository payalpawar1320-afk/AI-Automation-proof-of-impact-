...
export async function connectToDatabase(): Promise<typeof mongoose | null> {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(MONGODB_URI, opts).then((m) => {
      console.log('✅ Successfully connected to MongoDB Atlas Cluster0 (proof_of_impact db)');
      return m;
    }).catch(err => {
...
export const RecurringPatternModel = mongoose.models.RecurringPattern || mongoose.model('RecurringPattern', RecurringPatternSchema);
