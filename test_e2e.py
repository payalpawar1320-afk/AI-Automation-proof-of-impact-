...
def run_e2e_suite():
    ...
    # 1. Test Homepage
    home = test_endpoint("1. Frontend Home UI", f"{FRONTEND_URL}/")
    assert "<!DOCTYPE html>" in home or "html" in home or "Proof of Impact" in home, "Home page failed to render HTML"

    # 2. Test Issues List
    issues_res = test_endpoint("2. Issues API (Campus Domain)", f"{API_URL}/api/issues?domain=campus")
    ...
    triage_res = test_endpoint("3. Real-Time AI Triage & Duplicate Check", f"{API_URL}/api/ai/triage", method='POST', data=triage_req)
    ...
    create_res = test_endpoint("4. Report New Issue & Trigger Intake Loop", f"{API_URL}/api/issues", method='POST', data=create_req)
    ...
    claim_res = test_endpoint("5. Authority Resolution Claim", f"{API_URL}/api/issues/{new_issue_id}", method='PATCH', data=claim_req)
    ...
    verify_res = test_endpoint("6. AI Evidence Adjudication & Verification", f"{API_URL}/api/ai/verify", method='POST', data=verify_req)
    ...
    verify_fail_res = test_endpoint("7. AI Adjudication Sub-standard Patch Rejection Test", f"{API_URL}/api/ai/verify", method='POST', data=civic_failed_req)
    ...
    patterns_res = test_endpoint("8. Recurring Problem Detector & Predictive Patterns", f"{API_URL}/api/ai/patterns?domain=campus")
    ...
    depts_res = test_endpoint("9. Departmental Impact Scoring & True vs Claimed Metrics", f"{API_URL}/api/analytics/departments?domain=campus")
    ...
    query_res = test_endpoint("10. Generative Reporting & Executive Intelligence AI", f"{API_URL}/api/ai/query", method='POST', data=query_req)
